...
void CHud::RenderCursor()
{
	if(Client()->State() != IClient::STATE_DEMOPLAYBACK && m_pClient->m_Snap.m_pLocalCharacter)
	{
		...
		Graphics()->RenderQuadContainerAsSprite(m_HudQuadContainerIndex, m_aCursorOffset[CurWeapon], TargetPos.x, TargetPos.y, g_Config.m_XcCursorSizeMultiplier / 10.0f, g_Config.m_XcCursorSizeMultiplier / 10.0f);
		return;
	}
	...
	Graphics()->TextureSet(m_pClient->m_GameSkin.m_aSpriteWeaponCursors[CurWeapon]);
	Graphics()->RenderQuadContainerAsSprite(m_HudQuadContainerIndex, m_aCursorOffset[CurWeapon], TargetPos.x, TargetPos.y, Zoom * (g_Config.m_XcCursorSizeMultiplier / 10.0f), Zoom * (g_Config.m_XcCursorSizeMultiplier / 10.0f));
}
