...
void CPlayers::OnRender()
{
	...
	{
		if(ClientId == LocalClientId || !m_pClient->m_Snap.m_aCharacters[ClientId].m_Active || !IsPlayerInfoAvailable(ClientId) || ((g_Config.m_XcHideIgnoredInAnyCondition && m_pClient->m_aClients[ClientId].m_Hidden) || (m_pClient->m_aClients[ClientId].m_Hidden && GameClient()->IsOtherTeam(ClientId))))
		{
			continue;
		}
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
		if(!m_pClient->m_aClients[ClientId].m_SpecCharPresent || ((g_Config.m_XcHideIgnoredInAnyCondition && m_pClient->m_aClients[ClientId].m_Hidden) || (m_pClient->m_aClients[ClientId].m_Hidden && GameClient()->IsOtherTeam(ClientId))))
		{
			continue;
		}
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
		if(ClientId == RenderLastId || !m_pClient->m_Snap.m_aCharacters[ClientId].m_Active || !IsPlayerInfoAvailable(ClientId) || ((g_Config.m_XcHideIgnoredInAnyCondition && m_pClient->m_aClients[ClientId].m_Hidden) || (m_pClient->m_aClients[ClientId].m_Hidden && GameClient()->IsOtherTeam(ClientId))))
		{
			continue;
		}
